...
function convertFahrToCelsius (F) {
    ...
    if (toString.call(F) === "[object Array]") { 
        ...
        return result = `${arrayValue} is not a valid number but an Array.`
    }
    if (F instanceof Object) {
        let objectValue = JSON.stringify(F);
        return finallu = `${objectValue} is not a valid number but an object.`
    } else {
        result = `${JSON.stringify(F)} is not a valid but a/an ${inputData}.`
        return result;
    }
}
...
function checkYuGiOh(n) {
    ...
    try {
        let arr = [...new Array(Number(n))].map((_value, index) => {
            //increment index which start from 0 to start from 1 index++:
            ...
            else if (index % 2 === 0 && index % 3 == 0) return _2 + "_" + _3;
            else if (index % 2 === 0 && index % 5 === 0) return _2 + "_" + _5;
            else if (index % 3 === 0 && index % 5 === 0) return _3 + "_" + _5;
